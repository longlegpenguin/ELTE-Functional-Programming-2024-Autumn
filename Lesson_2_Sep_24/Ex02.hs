...
{- 
| <condition> = <expression> => if <condition> then return <expression>
maxi 34 36 =
if 34 > 36 => 34
otherwise => 36
 -}
...
{- 
mod => modular => take divisor's sign => want a number between 0 and the divisor.
rem => remainder => take dividend's sign => want a number between 0 and the dividend.
...
-- 7. Write a function which returns true if a is divisible by b or vice versa.
divAny :: Int -> Int -> Bool
divAny a b = rem a b == 0 || rem b a == 0
...
{- 
Recursion? => Do something repeatly by calling itself, until one certain (stop) condition is reached.
- Stop condition(s)
- Recursive call of the funciton itself.
 -}
...
{- 
fromIntegral:
    from Integral type class to an instance of the Num type class.
    Integral type class instances: Int, ...
    Num type class instances: Double, Float, Int, ...
sqrt:
    Float -> Float
floor:
    Real -> Integral
...
{- 
where:
    local bindings => binds functions or variables to the certain function.
 -}
...
sumOdd n
    ...
    | n == 1 = 1
    | (mod n 2 == 0) = sumOdd (n - 1)
    | (n `mod` 2 /= 0) = n + sumOdd (n - 2)
...
toDigit :: Int -> Int
toDigit 0 = 0
toDigit x = (mod x 10) ^ 3 + toDigit (div x 10)


armstrong :: Int -> Bool
armstrong x = toDigit x == x
