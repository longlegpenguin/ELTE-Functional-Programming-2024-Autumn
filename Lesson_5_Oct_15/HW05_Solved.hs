...
vowelUpper [] = []
vowelUpper (x: xs)
  | x == "" ||  not (isVowel (head x)) = vowelUpper xs
  | otherwise = (map toUpper x) : vowelUpper xs
...
transformer s n
  ...
  | s == "multiply" = (\x -> n * x)
  | s == "power" = (\x -> x^n)
  | otherwise = error "invalid input operation"
...
logicChain functions "and" = (\x -> foldl (\res func -> (func x) && res) True functions)
logicChain functions "or" = (\x -> foldl (\res func -> (func x) || res) False functions)
logicChain functions _ = error "logic string invalid"
...
transposeAndSum matrix func = (transpose matrix) ++ [map (\x -> sum (filter func x)) matrix]
...
maxLengthWords words = filter (\x -> maxLen == length x) words
  where maxLen = foldl maxHelper 0 words
